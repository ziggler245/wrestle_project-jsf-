 ...
 */
@ManagedBean(eager=true,name="backlash")
@RequestScoped
public class Backlash implements Serializable{
    ...
    public static void getBacklashPlan(){
         ...
         ppvPlan=new ArrayList();
            ...
            if (connection == null)
            {
                throw new SQLException("Error establishing connection!");
                 ...
                 query="SELECT *  FROM ppvs_seats where name=\"Backlash\"";
             ...
             query = "SELECT * from ppv_shows where name=\"Backlash\" group by seat ";
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
    ...
    for(int i=1;i<=cont;i++){
        ...
        if(BinarySearch.bin(BinSearch, i)==false){
             ...
             query = "SELECT * from ppv_shows where name=\"Backlash\" and seat="+i;
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
            ...
            Logger.getLogger(Backlash.class.getName()).log(Level.SEVERE, null, ex);
        }finally {
            try {
                if (statement != null) {
                    statement.close();

 ...
 */
@ManagedBean(eager=true,name="fastlane")
@RequestScoped
public class Fastlane implements Serializable{
    ...
    public static void getFastlanePlan(){
         ...
         ppvPlan=new ArrayList();
            ...
            if (connection == null)
            {
                throw new SQLException("Error establishing connection!");
                 ...
                 query="SELECT *  FROM ppvs_seats where name=\"Fastlane\"";
             ...
             query = "SELECT * from ppv_shows where name=\"Fastlane\" group by seat ";
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
 ...
 hallPlan.add(hp);
              ...
              query = "SELECT * from ppv_shows where name=\"Fastlane\" and seat="+i;
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
            ...
            Logger.getLogger(Fastlane.class.getName()).log(Level.SEVERE, null, ex);
        }finally {
            try {
                if (statement != null) {
                    statement.close();

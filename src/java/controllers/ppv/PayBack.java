 ...
 */
@ManagedBean(eager=true,name="payback")
@RequestScoped
public class PayBack implements Serializable{
    ...
    public static void getPayBackPlan(){
         ...
         ppvPlan=new ArrayList();
            ...
            if (connection == null)
            {
                throw new SQLException("Error establishing connection!");
                 ...
                 query="SELECT *  FROM ppvs_seats where name=\"PayBack\"";
             ...
             query = "SELECT * from ppv_shows where name=\"Payback\" group by seat ";
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
 ...
 hallPlan.add(hp);
              ...
              query = "SELECT * from ppv_shows where name=\"Payback\" and seat="+i;
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
            ...
            Logger.getLogger(PayBack.class.getName()).log(Level.SEVERE, null, ex);
        }finally {
            try {
                if (statement != null) {
                    statement.close();

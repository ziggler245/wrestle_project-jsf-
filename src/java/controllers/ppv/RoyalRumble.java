 ...
 */
@ManagedBean(eager=true,name="rrumble")
@RequestScoped
public class RoyalRumble implements Serializable{
    ...
    public static void getRoyalRumblePlan(){
         ...
         ppvPlan=new ArrayList();
            ...
            if (connection == null)
            {
                throw new SQLException("Error establishing connection!");
                 ...
                 query="SELECT *  FROM ppvs_seats where name=\"Royal Rumble\"";
             ...
             query = "SELECT * from ppv_shows where name=\"Royal Rumble\" group by seat ";
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
 ...
 hallPlan.add(hp);
             ...
             query = "SELECT * from ppv_shows where name=\"Royal Rumble\" and seat="+i;
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
            ...
            Logger.getLogger(RoyalRumble.class.getName()).log(Level.SEVERE, null, ex);
        }finally {
            try {
                if (statement != null) {
                    statement.close();

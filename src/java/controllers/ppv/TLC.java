 ...
 */
@ManagedBean(eager=true,name="tlc")
@RequestScoped
public class TLC implements Serializable{
         ...
         ppvPlan=new ArrayList();
            ...
            if (connection == null)
            {
                throw new SQLException("Error establishing connection!");
                 ...
                 query="SELECT *  FROM ppvs_seats where name=\"TLC\"";
             ...
             query = "SELECT * from ppv_shows where name=\"TLC\" group by seat ";
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
 ...
 hallPlan.add(hp);
              ...
              query = "SELECT * from ppv_shows where name=\"TLC\" and seat="+i;
             statement = connection.prepareStatement(query);
             rs = statement.executeQuery();
             while(rs.next()){
            ...
            Logger.getLogger(TLC.class.getName()).log(Level.SEVERE, null, ex);
        }finally {
            try {
                if (statement != null) {
                    statement.close();
